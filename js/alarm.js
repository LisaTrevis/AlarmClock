...
function setAlarm() {
	...
	// Takes value selected by user in hoursDropdown variable and turns it into an integer.
	var userHoursInput = parseInt(hoursDropdown.value);
	// Takes value selected by user in minutesDropdown variable and turns it into an integer.
	...
	var userMinsRaw = (userMinsInput < 10) ? "0" + userMinsInput : userMinsInput;

	var userMins = parseInt(userMinsRaw);
	...
	document.getElementById("message").textContent = "Alarm is now set for " + hoursDropdown.value + ":" + userMinsRaw + " " + ampm + ".";

	document.getElementById("message").className = "animated fadeInUp";
